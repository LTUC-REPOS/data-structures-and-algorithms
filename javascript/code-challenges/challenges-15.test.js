...
const screenForNames = (arr) => {
  return arr.filter(str => /^\b(Mr.\s\w+|Mrs.\s\w+|Ms.\s\w+|Dr.\s\w+)$/g.test(str));
};
...
let biggerThanLuke = (arr) => {
  return arr.filter(i => { return parseInt(i.mass) > parseInt(arr[0].mass); }).map(i => i.name).join(" - ");
...
const sortBy = (property, arr) => {
  ...
  return arr.sort((a, b) => {
    if (property === 'price') {
      let num1 = parseInt(a[property]);
      let num2 = parseInt(b[property]);
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  return /^\b(https:\/\/(www|).\w+.\w+)$/gi.test(url);
};
...
[    0   1    2
 ...
 2 ['X', 'O', 'X'],
  ...
  if i == j   check the 3 valid wins
  if i != j   check the 2 valid wins
...
const detectTicTacToeWin = (board) => {
  ...
  let countDig = 0;
  let otherDig = 0;
  let countRowCenter = 0;
  let countColCenter = 0;
  for (let j = 0; j < 3; j++) {
    if (board[0][j] == first && first != "") {
      // Counting First Row
      countX++;
    }
    if (board[j][0] == first && first != "") {
      // Counting First Col
      countY++;
    }
    if (board[j][j] == first && first != "") {
      // Counting First Dig
      countDig++;
    }
    if (board[1][j] == center && center != "") {
      // Count Row Center
      countRowCenter++;
    }
    if (board[j][1] == center && center != "") {
      // Count COL center
      countColCenter++;
    }
    if (board[0][2] == center && board[2][0] == center && center != "") {
      otherDig = 3;
    }
  }
  if (
