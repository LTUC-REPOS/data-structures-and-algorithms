...
const addPurchases = (arr) => {
  ...
  return arr.reduce((p, c) => {
    return p.purchasePrice + c.purchasePrice;
  });
};
...
const reversedString = (str) => {
  ...
  return str.split('').reduce((previousValue, current) => {
...
const extractStat = (statName, arr) => {
  return arr.reduce ( (oldValue, currentValue ) => {
    if (currentValue.stat.name === statName || currentValue.name === statName)
      return currentValue;
    else
      return oldValue;
  },null);
...
const extractChildren = (arr) => {
  ...
  let filtered = arr.filter(i=> i.name.includes('a') );
  let reduced = filtered.reduce( (oldResult,currentValue)=>{
    if ( currentValue.hasOwnProperty('children'))
      return [...oldResult ,... currentValue.children];
    else
      return oldResult;
  },[]);
