...
const listFoods = (recipe) => {
  ...
  for (const ingredient of recipe.ingredients) {
    let tillFirst = ingredient.slice(ingredient.indexOf(" ")+1);
    let tillSecond = tillFirst.slice(tillFirst.indexOf(" ")+1);
    result.push(tillSecond);
  }
...
const splitFoods = (recipe) => {
  let result = [];
  for (const ingredient of recipe.ingredients) {
    let tillSecond = ingredient.split(" ").slice(2).join(" ");
    result.push(tillSecond);
  }
  return result;
};
...
const stepActions = (recipe) => {
  ...
  for (const step of recipe.steps) {
    let action = step.slice(0,step.indexOf(" "));
    result.push(action);
  }
  return result;
};
...
const totalSumCSV = (str) => {
  let total = str.split(',').reduce( (total,c) => total + parseInt(c) , 0);
  return total;
};
...
const extractVowels = (str) => {
  ...
  return [notVowls.join("") , strVowls.sort().join("")];
};
...
describe('Testing challenge 12', () => {
  test('It should return the string without vowels', () => {
    ...
    expect(removeVowels('why')).toStrictEqual('wh');
  });
});
