...
const totalSum = (input) => {
  return input.join().split(',').reduce ((sum,val)=>{
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map( arr => {
    let tempArr= [];
    arr.forEach((num)=> (num%5 == 0 && num !=0 && typeof(num) == 'number') ? tempArr.push(2**num) : '');
    return tempArr;
  })
};
...
let findMaleAndFemale = (data) => {
  return data.reduce( (str,val)=>{
    str += (val.gender == 'male' ||val.gender == 'female') ? ' and ' + val.name : ''
    return str;
  },'').slice(5);
};
